...
from utils.amz_qnetworks import QNetworkConcat, QNetworkAttention
from utils.amz_utils import sort_states_for_snake_id
...
class Agent:
    ...
    def act(self, state, snake_id, turn_count, snake_health, episode, eps=0.):
        ...
        if random.random() > eps:
            ...
            with autograd.predict_mode():
                last_n_memory = self.memory.get_last_n(
    ...
    def learn(self, experiences, gamma):
        ...
        action_indices = nd.array(
            np.arange(0, last_action.shape[0])).as_in_context(ctx)
        action_indices.attach_grad()
        last_actions = nd.concat(
            action_indices.expand_dims(1), last_action, dim=1)
    ...
    def soft_update(self, tau):
        ...
        for target_name, local_name in zip(self.target_model_params, self.local_model_params):
            ...
            target_params.set_data(
                tau*local_params.data() + (1.0 - tau)*target_params.data())
...
class ReplayBuffer:
    ...
    def sample(self, sequence_length):
        ...
        indexes = random.sample(
            range(sequence_length, len(self.memory)-1), k=self.batch_size)
