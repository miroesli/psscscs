...
\begin{center}
  \hl{Note: Version 2 changes are highlighted in yellow.}
\end{center}
...
\begin{dmath}
  ...
  \mathcolorbox{yellow}{\sum\limits_{s_i}} \Psi(s_0,a,s_i)\max_{a_j}Q(s_i,a_j)
  - \Psi(\mathcolorbox{yellow}{s_0},a,s_1) \times \max_{a_i}Q(s_1,a) =
  \Psi(s_0,a,s_1) \times [\max_{a_i}Q^*(s_1,a_i) -
  \max_{a_i}\mathcolorbox{yellow}{Q(s_1,a_i)]+\sum\limits_{s_i}}
  \Psi(s_0,a,s_i)\max_{a_j}Q(s_i,a_j) = \Psi(s_0,a,s_1) \times
  [\max_{a_i}Q^*(s_1,a_i) - \max_{a_i}\mathcolorbox{yellow}{Q(s_1,a_i)}] +
  Q(s_0,a)
\end{dmath}
...
\begin{equation}
  \Psi(s_o,a,\Gamma(s_o,\langle a, a_1, \ldots , a_k \rangle )) \approx \Pi^k_{i=1}\pi_n{(\langle Q(s_i,0),Q(\mathcolorbox{yellow}{s_i},1),Q(s_i,2) \rangle)}_{a_i}
\end{equation}
...
\begin{enumerate}
  ...
  \item \hl{Consider using a better computing device to increase training efficiency}
\end{enumerate}
